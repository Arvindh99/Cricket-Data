...
team_name_mapping = {'Delhi Daredevils': 'Delhi Capitals', 'Kings XI Punjab': 'Punjab Kings', 'Royal Challengers Bangalore': 'Royal Challengers Bengaluru',
    'Rising Pune Supergiants': 'Rising Pune Supergiant', 'Gujarat Lions':  'Gujarat Titans'}
...
venue_mapping = {'M.Chinnaswamy Stadium': 'M Chinnaswamy Stadium','M Chinnaswamy Stadium, Bengaluru': 'M Chinnaswamy Stadium',
    ...
    'Wankhede Stadium, Mumbai': 'Wankhede Stadium','Eden Gardens, Kolkata':'Eden Gardens','Sawai Mansingh Stadium, Jaipur': 'Sawai Mansingh Stadium',
    'Dr DY Patil Sports Academy, Mumbai':'Dr DY Patil Sports Academy','Brabourne Stadium, Mumbai':'Brabourne Stadium','Punjab Cricket Association Stadium, Mohali': 'IS Bindra Stadium',
    ...
    'Narendra Modi Stadium, Ahmedabad':'Narendra Modi Stadium','Bharat Ratna Shri Atal Bihari Vajpayee Ekana Cricket Stadium, Lucknow': 'Ekana Cricket Stadium',
    'Dr. Y.S. Rajasekhara Reddy ACA-VDCA Cricket Stadium, Visakhapatnam': 'ACA-VDCA Cricket Stadium','ACA-VDCA Cricket Stadium': 'ACA-VDCA Cricket Stadium',
    'Himachal Pradesh Cricket Association Stadium, Dharamsala':'HPCA Stadium','HPCA Stadium':'HPCA Stadium',
    'Maharashtra Cricket Association Stadium, Pune':'MCA Stadium', 'Maharashtra Cricket Association Stadium':'MCA Stadium',
    'Sheikh Zayed Stadium':'Zayed Cricket Stadium, Abu Dhabi','Maharaja Yadavindra Singh International Cricket Stadium, New Chandigarh':'Maharaja Yadavindra Singh International Cricket Stadium',
...
HIGH_TOSS_IMPACT_VENUES = {'Dubai International Cricket Stadium', 'Sharjah Cricket Stadium', 'Zayed Cricket Stadium, Abu Dhabi','Wankhede Stadium', 'MA Chidambaram Stadium', 
    'Eden Gardens','Rajiv Gandhi International Stadium','M Chinnaswamy Stadium', 'Brabourne Stadium', 'Dr DY Patil Sports Academy', 'Narendra Modi Stadium', 'Ekana Cricket Stadium'}
...
total_matches = (train_data.groupby('team1').size().add(train_data.groupby('team2').size(), fill_value=0))
team_win_rate = (total_wins / total_matches).fillna(0)

data['strength_diff'] = (data['team1'].map(team_win_rate).fillna(0) - data['team2'].map(team_win_rate).fillna(0))
...
_train_ctx  = train_data.assign(match_context=(train_data['team1'].astype(str) + '_' +train_data['team2'].astype(str) + '_' +train_data['venue'].astype(str) + '_' +train_data['toss_winner'].astype(str)))
...
algorithms = {'Logistic Regression': {"model": Pipeline([('scaler', StandardScaler()),('classifier', LogisticRegression(solver='saga', max_iter=10000))]),
        "params": {"classifier__penalty":['l1', 'l2', 'elasticnet'], "classifier__C": [0.01, 0.1, 1, 10, 100],"classifier__l1_ratio": [0.0, 0.5, 1.0]}},
            
            'Decision Tree': {"model": Pipeline([('scaler', StandardScaler()),('classifier', tree.DecisionTreeClassifier())]),
        "params": {"classifier__criterion":['gini', 'entropy'], "classifier__max_depth":[None, 5, 10, 15, 20, 30], "classifier__min_samples_split": [2, 5, 10, 20],
            "classifier__min_samples_leaf":  [1, 2, 4, 8]}},
    
            'Random Forest': {"model": Pipeline([('scaler', StandardScaler()), ('classifier', RandomForestClassifier(random_state=42))]),
        "params": {"classifier__n_estimators": [100, 200, 300, 400], "classifier__max_features": ["sqrt", "log2", None], "classifier__max_depth": [None, 10, 20, 30, 40],
            "classifier__min_samples_split": [2, 5, 10], "classifier__bootstrap": [True, False]}},
    
            'NaiveBayes': {"model": Pipeline([('scaler', StandardScaler()),('classifier', GaussianNB())]),
        "params": {"classifier__var_smoothing": np.logspace(0, -9, num=10)}},
    
            'K-Nearest Neighbors': {"model": Pipeline([('scaler', StandardScaler()),('classifier', KNeighborsClassifier())]),
        "params": {"classifier__n_neighbors": [3, 5, 7, 10, 15, 20], "classifier__weights": ["uniform", "distance"], "classifier__p": [1, 2]}},
    
            'Gradient Boost': {"model": Pipeline([('scaler', StandardScaler()), ('classifier', GradientBoostingClassifier(random_state=42))]),
        "params": {"classifier__learning_rate": [0.01, 0.05, 0.1, 0.2],"classifier__n_estimators":[100, 200, 300],
            "classifier__min_samples_split": [2, 5, 10], "classifier__max_depth": [3, 5, 10],"classifier__subsample": [0.8, 0.9, 1.0]}},
    
            'XGBoost': {"model": Pipeline([('scaler', StandardScaler()),('classifier', XGBClassifier(eval_metric='mlogloss', use_label_encoder=False, random_state=42))]),
        "params": {"classifier__n_estimators": [100, 200, 300], "classifier__max_depth": [3, 5, 7], "classifier__learning_rate": [0.01, 0.05, 0.1],
            "classifier__subsample": [0.7, 0.8, 1.0], "classifier__colsample_bytree": [0.7, 0.9, 1.0], "classifier__reg_alpha": [0, 0.1, 0.5],
            "classifier__reg_lambda": [1, 1.5, 2]}},
    
            'LightGBM': {"model": Pipeline([('scaler', StandardScaler()),('classifier', LGBMClassifier(random_state=42, verbose=-1))]),
        "params": {"classifier__n_estimators": [100, 200, 300], "classifier__max_depth": [3, 5, 7, -1], "classifier__learning_rate": [0.01, 0.05, 0.1],
            "classifier__num_leaves": [20, 31, 50], "classifier__subsample": [0.7, 0.8, 1.0],"classifier__colsample_bytree": [0.7, 0.9, 1.0]}}}
...
for model_name, values in algorithms.items():
    print(f"\nTraining: {model_name} ...")
    try:
        rscv = RandomizedSearchCV(estimator=values["model"], param_distributions=values["params"], cv=cv, n_iter=30, n_jobs=-1, verbose=0, random_state=42, scoring='accuracy')
        rscv.fit(X_train, y_train)

        prediction_models[model_name] = rscv
        model_details.append({"Model Name": model_name, "CV Best Score": rscv.best_score_, "Best Parameters": rscv.best_params_})
...
for model_name, model in prediction_models.items():
    ...
    test_results.append({"Model Name": model_name, "Test Acc": model.score(X_test, y_test), "ROC-AUC": auc, "Precision": report["weighted avg"]["precision"],
...
ConfusionMatrixDisplay.from_estimator(best_model, X_test, y_test, display_labels=team_names_ordered, xticks_rotation=45,ax=ax)
